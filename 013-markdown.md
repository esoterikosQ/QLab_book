...
- <s>취소선</s> : '~' 두 개로 취소선을 넣으려는 글씨 앞과 뒤를 감쌉니다.
...
하이퍼링크는 인터넷 주소뿐만 아니라 문서 내의 목차 위계로도 연결할 수 있습니다. 이때에 링크 주소는 '#'와 목차 텍스트에 있는 빈칸 대신 '-'를 결합한 형태로 입력합니다. 예를 들어 "\[지원 소프트웨어\]\(\#지원-소프트웨어\)"라고 입력하면 [지원 소프트웨어](#supported-software)라는 링크가 생기고, 이 링크를 클릭하면 문서 내의 '지원 소프트웨어' 제목이 있는 위치로 이동합니다.
